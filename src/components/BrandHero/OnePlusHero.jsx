...
const OnePlusHero = () => {
    ...
    const fetchData = async () => {
        const response = await axios.get(`https://ukkh4uvf1d.execute-api.eu-north-1.amazonaws.com/api/products/680a6ec2a4ac4b8a9de862b7`);
        ...
        <section className='oneplus-brand'>
            ...
            <div className="description-hero">
                <div className="brand-description">
                    ...
                    <div className="brand-hero" style={{display: isMobile ? 'block' : 'none'}}>
                    <img src={oneplusImage} alt="OnePlus 13"/>
                    </div>
                    <h3>{productData.name}</h3>
                    {/* <img src={productData.image} alt="" /> */}
                    <p dangerouslySetInnerHTML={{ __html: formattedDescription }}></p>
                    <Link to={`/pdp`} state={{ productdata: productData }}>Know more </Link>
                </div>
                <div className="brand-hero" style={{display: isMobile ? 'none' : 'block'}}>
                    <img src={oneplusImage} alt="OnePlus 13" />
                </div>
            </div>
        </section>
